  ...
  get addFuseSearch() {
    const searchCtrl = L.control.fuseSearch({
      ...
      showResultFct: (feature, container) => {
        ...
        const details = props.nom;
        const winner = L.DomUtil.create("b", null, container);
        winner.innerHTML = `Winner ${props.winner}`;
